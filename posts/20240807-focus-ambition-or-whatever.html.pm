...
◊aside{As an aside, I think there is a toxic undercurrent of constant comparison, overwork, and self-flagellation that runs in academia and that drives people away. We can do without it. At least I can. But it's also difficult, of course---in a competitive system that is driven largely by prestige (i.e. there are only so many papers that can be accepted at top conferences and so many positions at research institutions), it's hard to get away from all of that. It's important to try though, both on a personal and a systemic level.}
