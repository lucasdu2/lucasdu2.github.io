...
For what it's worth, the general argument for polynomial time complexity is basically as follows: (1) there are a linear number of layers in our layered graph; (2) at each layer, there are 4 stages, each of which can be completed in polynomial time; (3) thus each layer can be completed in polynomial time, (4) meaning the entire algorithm is polynomial time. Finally, (5) all points-to relations (i.e. the point of the algorithm) can simply be read off the graph resulting from the algorithm in constant time, since they are just the edges.
